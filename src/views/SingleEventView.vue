...
<template>
  <div class="event-container">
    ...
    <h1 class="event-title">{{event.name}}</h1>
      ...
      <div class="event-text" v-if="event.signUpURL">To sign up, follow <a :href="event.signUpURL" target="_blank" rel="noopener noreferrer">this link</a></div>
    </div>
...
export default {
  ...
  computed: {
    eventName: function() {
      return this.$route.params.name.replace(/-/ig, ' ')
    },
    event: function() {
